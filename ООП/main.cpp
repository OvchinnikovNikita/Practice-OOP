...
{
private:
	static const int MAXROW = 200; // Ограничение на максимальное количество вершин в некоторых алгоритмах. В случае необходимости изменить
	...
	// Переменные используются в алгоритме поиска шарниров и мостов
	int time = 0, t[MAXROW] = { 0 }, f[MAXROW] = { 0 }; /* t - массив времени захода в вершину
	...
	void Bridges_search_dfs(int v, vector<bool>& used_v, int p = -1)
	{
		// Алгоритм схож с алгоритмом поиска шарниров. Начало аналогично
		used_v[v] = true;

		t[v] = f[v] = time++;

		// Все условия взяты из описания алгоритма. 
		for (int i = 0; i < vec_Adjacencylist[v].size(); ++i) {

			int to = vec_Adjacencylist[v][i];

			if (to == p) // Не расматриваем предка
			{
				continue;
			}
			...
			{
				Bridges_search_dfs(to, used_v, v); // вызываем алгоритм уже от новой вершины
				f[v] = min(f[v], f[to]);

				if (f[to] > t[v]) // Если мост, то выведем его концы 
					cout << "(" << v + 1 << "," << to + 1 << ")" << endl;
			}
		}
	}
...
public:
	...
	void Bridges_search()
	{
		...
		cout << "Мосты данного графа:" << endl;
		for (int i = 0; i < row; ++i) // от каждой вершины. 
			if (!used.at(i))
				Bridges_search_dfs(i, used_v); // Основные процессы протекают в этой функции. Приват класса. строка 88
	}
};
...
int main()
{
	...
	Graph graph;
	graph.Set_Adjacencylist();  // Считывает матрицу 
